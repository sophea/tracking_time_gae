            ...
            if (!crudIsAuditField(key)) {
            	if(item == "boolean") {
            		...
            		$("#createFieldset").append("<label for='create_" + key + "'>" + key + "</label><br/>" +
                    		...
                    		"<span class='input-group-addon'><span class='glyphicon glyphicon-th'></span></span></div><br/>");
            	}
            	else{
...
function crudIsAuditField(key) {
   return "createdBy" == key || "createdDate" == key || "updatedBy" == key || "updatedDate" == key || "state" == key || "version" == key; 
}
...
function sticky_relocate() {
    ...
    console.log("win_top "+window_top + "  div_top: "+div_top);
    if (window_top > div_top) {
        $('#sticky').addClass('container stick');
